...
| **Los juegos de EVA ARCADE** (FORO [in]VISIBLE y EXPEDIENTE 1725, otros repositorios) | `lib/marca-eva.ts` de cada juego, generado desde aquí con `--kit` | El símbolo □X con «EVA ARCADE»: botón de vuelta a `/links` en la portada y en la cabecera (desde 768 px); en Procesal reemplaza el rombo provisional de EVA; en Familia, el ƎVΛ dibujado a mano. Iconos y vista previa para redes con el color de su tarjeta en `/links`. HANDOFF §0.0.7. |

 ...
 *
 * El tope alto es el de la v9: antes 78 puntos. El coste del campo no está
 * en los puntos sino en los hilos, que se prueban por pares: 78 puntos son
 * 3.003 medidas por fotograma y 46 son 1.035. Se ve casi igual —el fondo es
 * una insinuación— y el hilo principal lo agradece. Los otros dos niveles
 * bajan desde ahí.
 */
const DENSITY: Record<QualityLevel, { max: number; perPixel: number; link: number }> = {
  high: { max: 46, perPixel: 34000, link: LINK_DISTANCE },
  mid: { max: 36, perPixel: 44000, link: 116 },
  low: { max: 24, perPixel: 60000, link: 100 },
};
 ...
 * Vigilancia; hoy no hay ninguno), y las acciones del genoma lo sacuden un
 * instante. La quietud es una frenada que se ve ocurrir, no un interruptor.
 *
 * Lo que cuesta, y por qué se hace así (v9):
 * - Cede ante el campo de la Consciencia (`fieldSignal.yielded`): se funde a
 *   nada y deja de calcular mientras aquel está en pantalla. Dos campos a la
 *   vez eran dos ideas compitiendo, y dos bucles.
...
export function EvaField({ particles = true }: { particles?: boolean }) {
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      // La luz sigue al puntero aunque el campo haya cedido: es otra capa.
      placeLight();
       ...
       * pidiendo —cuesta una comparación— para retomar sin salto al volver, y
       * para que el sensor de calidad siga midiendo.
       */
